document.addEventListener('DOMContentLoaded', function () {
    ...
    const listPostsButton = document.getElementById('listPostsButton');
    const viewPostForm = document.getElementById('viewPostForm');
    const searchPostsByUserForm = document.getElementById('searchPostsByUserForm');
    // Data mock for users
    let users = [
        {
            id: 1,
    ...
    loginForm.addEventListener('submit', function (event) {
        ...
        const email = loginForm.elements['loginEmail'].value.trim();
        const password = loginForm.elements['loginPassword'].value.trim();
        if (email && password) {
            const user = users.find(u => u.email === email && u.password === password);
            if (user) {
                document.getElementById('loginMessage').textContent = `Xin chào ${user.first_name} ${user.last_name}!`;
            } else {
                document.getElementById('loginMessage').textContent = "Thông tin tài khoản không chính xác.";
            }
        } else {
            document.getElementById('loginMessage').textContent = "Hãy nhập đầy đủ thông tin.";
        }
    });
    ...
    registerForm.addEventListener('submit', function (event) {
        event.preventDefault();
        const firstName = registerForm.elements['registerFirstName'].value.trim();
        const lastName = registerForm.elements['registerLastName'].value.trim();
        const email = registerForm.elements['registerEmail'].value.trim();
        const password = registerForm.elements['registerPassword'].value.trim();
        if (firstName && lastName && email && password) {
            const existingUser = users.find(u => u.email === email);
            if (existingUser) {
                document.getElementById('registerMessage').textContent = "Email này đã có tài khoản.";
            } else {
                const newUser = {
                    id: users.length + 1,
            ...
            document.getElementById('registerMessage').textContent = "Hãy nhập đầy đủ thông tin.";
        }
    });
    ...
    searchUsersForm.addEventListener('submit', function (event) {
        event.preventDefault();
        const keyword = searchUsersForm.elements['searchKeyword'].value.trim().toLowerCase();
    ...
    function displayUsers(users) {
        ...
        if (users.length === 0) {
            searchUsersResult.innerHTML = '<p>Không tìm thấy người dùng.</p>';
        } else {
            users.forEach(user => {
                const userHtml = `
                    <p>ID: ${user.id}, Họ và tên: ${user.first_name} ${user.last_name}, Email: ${user.email}</p>
                `;
                searchUsersResult.innerHTML += userHtml;
            });
        }
    }

    // Function to list all posts
    listPostsButton.addEventListener('click', function () {
        displayPosts(posts);
    });

    function displayPosts(posts) {
        const listPostsResult = document.getElementById('listPostsResult');
        listPostsResult.innerHTML = '';
        if (posts.length === 0) {
            listPostsResult.innerHTML = '<p>Không có bài đăng nào.</p>';
        } else {
            posts.forEach(post => {
                const user = users.find(u => u.id === post.user_id);
                const postHtml = `
                    <p>ID: ${post.id}, Tiêu đề: ${post.title}, Ngày tạo: ${post.created_at}, Người tạo: ${user.first_name} ${user.last_name}</p>
                `;
                listPostsResult.innerHTML += postHtml;
            });
        }
    }

    // Function to view post by ID
    viewPostForm.addEventListener('submit', function (event) {
        event.preventDefault();
        const postId = parseInt(viewPostForm.elements['viewPostId'].value.trim());
        const post = posts.find(p => p.id === postId);
        if (post) {
            const user = users.find(u => u.id === post.user_id);
            const viewPostResult = document.getElementById('viewPostResult');
            viewPostResult.innerHTML = `
                <p>ID: ${post.id}</p>
                <p>Tiêu đề: ${post.title}</p>
                <p>Nội dung: ${post.content}</p>
                <p>Link ảnh: ${post.image}</p>
                <p>Người tạo: ${user.first_name} ${user.last_name}</p>
                <p>Ngày tạo: ${post.created_at}</p>
                <p>Ngày sửa đổi: ${post.updated_at}</p>
            `;
        } else {
            document.getElementById('viewPostResult').innerHTML = '<p>Không tìm thấy bài đăng.</p>';
        }
    });

    // Function to search posts by user email
    searchPostsByUserForm.addEventListener('submit', function (event) {
        event.preventDefault();
        const userEmail = searchPostsByUserForm.elements['userEmail'].value.trim();
        const user = users.find(u => u.email === userEmail);
        if (user) {
            const userPosts = posts.filter(post => post.user_id === user.id);
            displayUserPosts(userPosts);
        } else {
            document.getElementById('searchPostsByUserResult').innerHTML = '<p>Không tìm thấy người dùng.</p>';
        }
    });

    function displayUserPosts(posts) {
        const searchPostsByUserResult = document.getElementById('searchPostsByUserResult');
        searchPostsByUserResult.innerHTML = '';
        if (posts.length === 0) {
            searchPostsByUserResult.innerHTML = '<p>Không có bài đăng nào của người dùng này.</p>';
        } else {
            posts.forEach(post => {
                const user = users.find(u => u.id === post.user_id);
                const postHtml = `
                    <p>ID: ${post.id}, Tiêu đề: ${post.title}, Ngày tạo: ${post.created_at}, Người tạo: ${user.first_name} ${user.last_name}</p>
                `;
                searchPostsByUserResult.innerHTML += postHtml;
            });
        }
    }
